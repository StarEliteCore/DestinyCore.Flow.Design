...
// // 存放所有可以被作为依赖项的类
// const destinycoreIocPool: Array<Function> = [];
// // 标记可被注入类
...
//     if (destinycoreIocPool.indexOf(_constructor) !== -1) {
//         return;
//     } else if (paramsTypes.length) {
//         paramsTypes.forEach((v, i) => {
//             if (v === _constructor) {
//                 throw new Error('不可以依赖自身');
//             } else if (destinycoreIocPool.indexOf(v) === -1) {
//                 throw new Error(`依赖${i}[${(v as any).name}]不可被注入`);
//             }
//         });
//     }
//     destinycoreIocPool.push(_constructor);
// }
...
//         // 参数不可注入
//         if (destinycoreIocPool.indexOf(v) === -1) {
